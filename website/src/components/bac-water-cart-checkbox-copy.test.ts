// ---------------------------------------------------------------------------
// "COMPLETE YOUR ORDER — [x] Add 10 mL Recon Water   +$14.99"
//
// …directly underneath a cart line reading "Recon Water (0.9% Benzyl Alcohol),
...
describe("bacWaterCheckboxCopy", () => {
  it("offers the size when it is not in the cart", () => {
    ...
    expect(copy.label).toBe("Add 10 mL Recon Water");
    expect(copy.price).toBe("+$14.99");
    expect(copy.ariaLabel).toBe("Add 10 mL Recon Water to your order");
  });
  ...
  it("reports the size as already in the cart rather than offering it again", () => {
    ...
    expect(copy.label).toBe("10 mL Recon Water — in your cart");
    ...
    expect(copy.price).toBe("$14.99");
    expect(copy.ariaLabel).toBe("Remove 10 mL Recon Water from your order");
  });
});
