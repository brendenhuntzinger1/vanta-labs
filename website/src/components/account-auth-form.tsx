...
export function AccountAuthForm() {
  ...
  // Apple sign-in via OAuth. Supabase redirects to Apple and back to the login
  // page, where the mount effect above finishes establishing our session cookie
  // (same path as the email-verification return).
  const handleOAuth = async (provider: "apple") => {
    ...
    try {
      const { error: oauthError } = await supabase.auth.signInWithOAuth({
        provider,
        options: { redirectTo: getEmailRedirectUrl(`/account/login?next=${encodeURIComponent(nextPath)}`) },
      });
      if (oauthError) {
      ...
      setError(submitError instanceof Error ? submitError.message : `Couldn't start ${provider} sign-in.`);
      setLoading(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="vl-panel mx-auto w-full max-w-md rounded-[1.75rem] p-6 sm:p-8">
      ...
      <div className="mt-5 space-y-4">
        <button
          ...
          disabled={loading}
          className="flex w-full items-center justify-center gap-3 rounded-xl border border-white/15 bg-black px-4 py-3 text-sm font-medium text-white transition hover:bg-zinc-900 disabled:opacity-60"
        >
