...
// worse than the bug.
// ---------------------------------------------------------------------------
export function CartProvider({ children, signedIn = false, emailGrant = false }: { children: React.ReactNode; signedIn?: boolean; emailGrant?: boolean }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      // so this costs nothing and skips work that could never succeed.
      //
      // A SHOPPER WHO ARRIVED FROM AN EMAIL IS NOT SIGNED IN AND STILL SHOPS.
      //
      // All three click trackers — cart recovery, campaigns, automations — mint
      // a browse grant, and the wall consults it last, so a grant-holder reaches
      // /cart with no account. This effect is where the STORE'S OWN TERMS
      // arrive, the live shipping configuration among them, so returning early
      // on `!signedIn` left every one of those shoppers reading
      // DEFAULT_SHIPPING_CONFIG: "Free shipping at $200.00 — $108.82 away" on a
      // store shipping every order free, under an email that had just said
      // shipping was free.
      //
      // `emailGrant` IS RESOLVED IN THE ROOT LAYOUT, NOT SNIFFED HERE. The
      // first fix read the recovery attribution cookie in the browser, which
      // works for exactly one of the three journeys: the campaign and
      // automation grants are httpOnly, so a client-side check is blind to them
      // and they kept the bug. The server can see all three.
      //
      // It still only decides whether to ASK. The endpoint authenticates the
      // request itself and answers 401 to a forged cookie, and the `response.ok`
      // guard below turns that back into the old no-op.
