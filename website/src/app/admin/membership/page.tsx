...
import { getMembershipAnalytics, listMembershipTiersAdmin, listPromotionalEvents } from "@/lib/admin-membership";
...
export default async function AdminMembershipPage() {
  ...
  const [tiers, events, analytics, bonusSettings] = canManage
    ? await Promise.all([
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <section className="vl-panel rounded-[1.8rem] p-5 sm:p-7">
          ...
          <p className="mt-3 max-w-3xl text-sm text-zinc-400 sm:text-base">
            Manage tiers, points bonuses, promotional events, and customer balances. No payment processor is
            connected yet, so paid-tier billing cannot actually charge or recur — memberships are activated and
            managed here manually until that integration exists.
          </p>
        </section>

        {canManage && analytics && bonusSettings ? (
