...
--
-- It is built ENTIRELY from idempotent statements ("create ... if not exists",
-- "add column if not exists", "create or replace function"), so it is:
--   * SAFE on a fresh database AND on your existing one,
--   * SAFE to re-run any number of times,
--   * never drops, overwrites, or reorders anything you already have.
--
-- It bundles three things the app needs, in dependency order:
--   1. schema-complete-sync.sql   (every table/column/index + coupon RPC)
--   2. inventory_items table      (partner-portal ops summary reads this)
--   3. validate_referral_code()   (client-safe referral discount lookup)
-- ============================================================================


-- ############################################################################
-- ## PART 1 of 3 — COMPLETE SCHEMA SYNC
-- ############################################################################
...
-- ############################################################################
-- ## PART 2 of 3 — inventory_items (partner-portal operations summary)
-- ############################################################################
...
-- ############################################################################
-- ## PART 3 of 3 — validate_referral_code() RPC
-- ############################################################################
