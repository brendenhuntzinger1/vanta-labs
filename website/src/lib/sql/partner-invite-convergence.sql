...
begin
  ...
  if exists (select 1 from pg_roles where rolname='anon') then
    execute $q$revoke all on function public.create_partner_invite(uuid, uuid, text, text, text, numeric, uuid) from public, anon, authenticated;$q$;
  end if;
  if exists (select 1 from pg_roles where rolname='service_role') then
    execute $q$grant execute on function public.create_partner_invite(uuid, uuid, text, text, text, numeric, uuid) to service_role;$q$;
  end if;
end
$rpc_lockdown$;
