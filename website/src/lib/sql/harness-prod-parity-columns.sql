...
alter table public.coupons add column if not exists member_scope text default 'all'::text;
...
alter table public.orders add column if not exists shippo_sync_attempts integer default 0;
...
alter table public.product_doses add column if not exists incoming_quantity integer default 0;
...
alter table public.products add column if not exists incoming_quantity integer default 0;
alter table public.products add column if not exists requires_reconstitution boolean default false;
...
alter table public.shippo_webhook_events add column if not exists retry_count integer default 0;
