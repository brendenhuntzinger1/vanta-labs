...
--   Applies to all three GLP compounds: semaglutide, tirzepatide, retatrutide.
...
update public.product_doses d set price_cents=4999,  updated_at=now()
 ...
 where p.id=d.product_id
   and p.slug in ('glp-1-semaglutide','glp-2-tirzepatide','glp-3-retatrutide')
...
select p.slug, d.slug_suffix, d.price_cents
 ...
 where p.slug in ('glp-1-semaglutide','glp-2-tirzepatide','glp-3-retatrutide')
