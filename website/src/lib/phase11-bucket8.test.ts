...
// member-savings awaits the builder itself after a PAGED `.order().range()`.
// One chainable object with both endings therefore serves both, and `state`
// decides what each ending answers.
//
// `range()` SLICES, and that is not decoration. It used to return the builder
// and ignore its bounds, so every page re-served the whole array —
// readAllRowsBounded then accumulated the same rows until it hit maxRows and
// correctly reported `truncated`, which getLifetimeSavings correctly turns into
// UNAVAILABLE. The stub was wrong and the production code was right, which is
// the only reason that showed up as a red test rather than a shipped bug.
// ---------------------------------------------------------------------------
  ...
  const chain = (single: () => Row, scan: () => Row) => {
    ...
    const b: Row = {
      ...
      then: (ok: (v: Row) => unknown, err?: (e: unknown) => unknown) => {
        ...
        // A ranged read gets the slice it asked for; an un-ranged one gets
        // everything, exactly as PostgREST behaves.
        const sliced = window && Array.isArray(rows)
          ? { ...answer, data: rows.slice(window[0], window[1] + 1) }
          : answer;
        return Promise.resolve(sliced).then(ok, err);
      },
    };
    return b;
  };
