...
// once catalog, membership and settings are supplied above — but importing them
// pulls in supabase-server, which throws on a missing NEXT_PUBLIC_SUPABASE_URL.
// This used to come free from a vi.mock in vitest.setup.ts that applied to all
// 200+ suites; that global was removed (finding E-A-01) because it also stubbed
// modules these tests meant to exercise. Applied here, where it is needed and
// visible.
vi.mock("@/lib/supabase-server", async () => {
  const { supabaseServerModule } = await vi.importActual<typeof import("@/test-support/payment-suite-fakes")>(
    "@/test-support/payment-suite-fakes",
  );
