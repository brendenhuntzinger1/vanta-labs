...
export async function createAdminCoupon(input: CouponInput): Promise<AdminCoupon> {
  ...
  // is_private is a newer column (coupon-private-flag.sql). If the migration
  // hasn't been applied yet, retry without it so coupon creation never breaks
  // — but ONLY for a non-private coupon: silently creating a "private" code
  // as public would advertise it to the whole store, the exact opposite of
  ...
  if (error && error.code !== "23505") {
    ...
    const missingColumn = error.code === "PGRST204"
      || (message.includes("is_private") && (message.includes("does not exist") || message.includes("schema cache") || message.includes("could not find")));
    if (missingColumn) {
      if (input.isPrivate) {
