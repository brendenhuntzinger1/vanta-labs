...
export type CouponInput = {
  ...
  active?: boolean;
  isPrivate?: boolean;
  /** Also waives shipping, on top of whatever the code takes off. */
  freeShipping?: boolean;
  memberScope?: "all" | "members" | "non_members";
};
...
export async function createAdminCoupon(input: CouponInput): Promise<AdminCoupon> {
  ...
  let { data, error } = await supabaseAdmin
    .from("coupons")
    .insert({ ...baseRow, is_private: Boolean(input.isPrivate), member_scope: memberScope, free_shipping: Boolean(input.freeShipping) })
    .select("*")
    .single();
  ...
  if (error && error.code !== "23505") {
    ...
    const missingColumn = error.code === "PGRST204"
      || ((message.includes("is_private") || message.includes("member_scope") || message.includes("free_shipping")) && (message.includes("does not exist") || message.includes("schema cache") || message.includes("could not find")));
    if (missingColumn) {
      if (input.isPrivate) {
      ...
      // Same rule as the two above, for the same reason: silently creating a
      // code WITHOUT the free shipping the operator asked for would promise
      // customers something the checkout then refuses to honour.
      if (input.freeShipping) {
        throw new Error("Free-shipping coupons need a one-time database update: run src/lib/sql/coupon-free-shipping.sql in Supabase, then try again.");
      }
