...
- [ ] Step 2: Implement and wire; commit `Omnisend: consent, cart, checkout and product-view hooks`.
  *Amended 2026-09-16.* This step originally said `onCheckoutStarted` mints the recovery code when `ensureNoRecentPurchase(email, 30d)`. It does not: `started checkout` mints nothing. The recovery code and gift are minted by the `omnisend_cart_offers` sweep (`cart-offers.ts`) 36 to 96 hours after the cart's last activity, at the cart-value band's percentage from `planStageOffer` (stage `t72h`), with a five-day code TTL (`CONTACT_CODE_OFFERS.recovery`), only for a cart with no in-house stage; the message reaches only a subscribed contact because every flow's sending threshold is `email: subscribed`. `onProductViewed` is debounced to one `viewed product` per address and product per six hours (`PRODUCT_VIEW_DEBOUNCE_MS`), not per hour.
