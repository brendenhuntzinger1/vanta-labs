# BLOCK E-B — Test quality (session `claude/audit-blocks-f-e-8jxi9v`)
...
| Commission tier | seed `matched` from `tiers[0]`, greps intact | ✅ **survived** | E-B-02 |
| Commission tier | neutralise the locked-rate return, grep intact | ✅ **survived** | E-B-02 |
| Payout authority | delete `canManageRefunds` on the payout route | ✅ **survived** | E-B-03 |
| Payout authority | hardcode `confirmedTransferred: true` | ✅ **survived** | E-B-03 |
| Payout authority | hardcode `overrideMinimumThreshold: true` | ✅ **survived** | E-B-03 |
| Payout authority | delete the minimum-threshold guard | ✅ **survived** | E-B-03 |
| Payout authority | delete the "still approved?" guard | ✅ **survived** | E-B-03 |
| Inventory decrement | strict-`false` oversell check → `null` | ❌ caught (2) | covered |
| Inventory decrement | degraded finalize reports phantom success | ✅ **survived** | E-B-04 |
...
| Email dedupe | delete the 23505 once-claim | ❌ caught (2) | covered, but see E-B-06 |
| Fulfillment state | email on every carrier scan, not the transition | ❌ caught (3) | covered |

Plus, found while fixing Block F: **E-B-01**, a grep that went red on a safe
refactor and would have stayed green on the defect it names.

---

## E-B-01 — A placebo that failed on a refactor and would pass on the bug
...
## E-B-02 — The commission rate an ambassador is paid had no behavioural test
...
## E-B-03 — Nobody could send an ambassador money wrongly and be noticed
...
## E-B-04 — A broken inventory RPC could report success and take nothing off the shelf
...
## E-B-05 — The function that takes membership money had zero coverage
...
## E-B-06 — Findings that DISPROVE parts of the map
...
## E-B-07 — Coupon fuzzing that was fuzzing nothing
...
## E-B-08 — The eleven global stubs are now documented
...
their own right (E-B-05, E-B-07). One is dead: `@/lib/fulfillment/service` mocks a
